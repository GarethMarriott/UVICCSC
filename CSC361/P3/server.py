...
FILE        = ""
FILE_SIZE   = 0
CACHED_FILE = "" # TODO: IMPLEMENT A FILE STORED IN MEMORY AND INDEX INTO WITH STRING
...
while(True):
    try:
        ...
        if header['syn'] and header['ack']:
            ...
            CASHED_FILE = repr(FILE.read())
            FILE_SIZE = len(CASHED_FILE)
            payload_to_client   = get_file_data(0 , 2000)
            ...
            print(FILE_SIZE)
            payload_to_client   = get_file_data(int(header['ack_number'],2) , 2000)
            ...
            if len(payload_to_client) == 0:
                ...
                msgFromServer       = encode_header(['data'],int(header['ack_number'],2),int(header['seq_number'],2),int(len(payload_to_client.encode('utf-8'))))
